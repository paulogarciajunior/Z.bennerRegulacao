...
def lerGuias():
    ...
    def dateToStr(o):
        if isinstance(o, datetime):
            return o.__str__()
    def getControle(guia,operadora):
        return {'guia':guia,
                'registroans':operadora,
                'sla':'24h',
...
#    cursor.execute('delete from neobeDES.guia')
    ...
    for guia in guias:
        ...
        cursor.execute("select * from neobeORIG.guiaeventos where idguia = {v_idguia}".format(v_idguia=guia["id"]))
        eventos = cursor.fetchall()
        guia["eventos"]=eventos

        conteudo = {
            'guia':{'com.benner.regulacao.guia':guia},
            'controle':{'com.benner.regulacao.controle':getControle(guia["id"],guia["registroans"])}
        }
        #print (json.dumps(conteudo))
